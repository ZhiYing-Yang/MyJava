...
public class Scene {// 把已有的类组织 起来 一起完成任务
	...
	/*
	 * get方法没有方法参数，因为他的作用不是传参数，而是获得方法返回值的 获取方法返回值，方法名之前就要有返回值的类型，此处为IceBox类型
	 * return语句必须有，没有会出错，因为该方法要求有返回值，没有返回语句类型都不对 传参用set 获得参数用get
	 */
	...
	/*
	 * set方法是传参的，因此返回值类型必须为空，void 类型。两个aIceBox并不是一回事，左边的是本类的成员 右边的是方法的参数变量， set
	 * 方法和get 方法功能
	 */
	/*
